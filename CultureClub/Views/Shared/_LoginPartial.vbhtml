...
@If Request.IsAuthenticated
    @Using Html.BeginForm("LogOff", "Account", FormMethod.Post, New With { .id = "logoutForm", .class = "navbar-right" })
        ...
        @<ul class="nav navbar-nav navbar-right">
            <li>
                @Html.ActionLink("Hello " + CType(User.Identity,ClaimsIdentity).FindFirstValue(ClaimTypes.GivenName) + "!", "Index", "Manage", routeValues:=Nothing, htmlAttributes:=New With {.title = "Manage"})
            </li>
